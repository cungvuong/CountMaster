...
public static class SimplePool {
	...
	class Pool {
        ...
        public GameObject GetLastObjActive()
        {
            if (active.Count > 0)
	...
	static public GameObject GetLastActiveobj(GameObject prefab)
    {
        Pool pool_temp = null;
        pools.TryGetValue(prefab, out pool_temp);
        if (pool_temp!=null)
            return pool_temp.GetLastObjActive();
        else
            return null;
    }
	...
	/// </summary>
	static public void Despawn(GameObject obj, bool _forceDespawn=false) {
		PoolMember pm = obj.GetComponent<PoolMember>();
		if(pm == null) {
			Debug.Log ("Object '"+obj.name+"' wasn't spawned from a pool. Destroying it instead.");
			GameObject.Destroy(obj);
		}
		else if(obj.activeSelf|| _forceDespawn) {
